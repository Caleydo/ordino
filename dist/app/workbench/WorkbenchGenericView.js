...
export function WorkbenchGenericView({ workbenchIndex, view, chooserOptions, mosaicDrag, path, removeCallback, isTransitioning, }) {
    var _a;
    ...
    return (React.createElement(MosaicWindow, { path: path, title: view.name, renderToolbar: () => header },
        React.createElement("div", { id: view.id, className: `position-relative flex-column shadow bg-body workbenchView rounded flex-grow-1 ${mosaicDrag ? 'pe-none' : ''} ${isTransitioning ? 'd-none tdp-busy' : ''}` },

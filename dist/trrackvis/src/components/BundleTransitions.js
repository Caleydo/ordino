...
export default function bundleTransitions(xOffset, yOffset, clusterOffset, backboneOffset, duration = 500, expandedClusterList, stratifiedMap, nodeList, annotationOpen, annotationHeight, bundleMap) {
    xOffset = -xOffset;
    ...
    const enter = (data) => {
        ...
        if (bundleMap && expandedClusterList.includes(data)) {
            for (let i = 0; i < bundleMap[data].bunchedNodes.length; i++) {
                ...
                //   validity = false;
                // }
                if (stratifiedMap[bundleMap[data].bunchedNodes[i]] &&

export default function linkTransitions(xOffset, yOffset, clusterOffset, backboneOffset, duration = 500, annotationOpen, annotationHeight) {
    xOffset = -xOffset;
    ...
    const enter = (data) => {
        ...
        let y1 = yOffset * source.depth -
            (yOffset - clusterOffset) * clusteredNodesInFront;
        let y2 = yOffset * target.depth -
            (yOffset - clusterOffset) * clusteredNodesInFront;
        if (annotationOpen !== -1 &&
...
export function getX(width, xOffset, backboneOffset) {
    return width > 1

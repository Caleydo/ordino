...
export function ProvVisCreator(node, prov, callback, buttons = true, ephemeralUndo = false, fauxRoot = prov.graph.root, config = {}) {
    prov.addGlobalObserver(() => {
        ReactDOM.render(React.createElement(ProvVis, Object.assign({}, config, { root: fauxRoot, changeCurrent: callback, current: prov.graph.current, nodeMap: prov.graph.nodes, prov: prov, undoRedoButtons: true, ephemeralUndo: ephemeralUndo })), node);
    });
    ReactDOM.render(React.createElement(ProvVis, Object.assign({}, config, { root: fauxRoot, changeCurrent: callback, current: prov.graph.current, nodeMap: prov.graph.nodes, prov: prov, undoRedoButtons: true, ephemeralUndo: ephemeralUndo })), node);
}

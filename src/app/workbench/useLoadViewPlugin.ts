...
import {AView, EXTENSION_POINT_TDP_VIEW, IViewPluginDesc, LoginMenu, PluginRegistry, useAsync, IView, ObjectRefUtils, ResolveNow, IDType, LocalStorageProvenanceGraphManager, ARankingView, IDTypeManager, FindViewUtils, IDiscoveredView, ISelection} from 'tdp_core';
...
export function useLoadViewPlugin(viewId: string, workbenchIndex: number): [(element: HTMLElement | null) => void, IView | null] {
    ...
    const setRef = React.useCallback(async (ref: HTMLElement | null) => {
        ...
        if (ref && status === 'success') {
            ...
            const selection: ISelection = {idtype: new IDType(idType, viewId, '', true), ids: workbenchIndex === 0 ? [] : ordino.workbenches[workbenchIndex - 1].selection};

...
export function HeaderNavigation({fixed, bg = 'dark', testId: parentTestId}: IHeaderNavigationProps) {
  ...
  return (
    <nav className={`ordino-header-navigation navbar navbar-expand-lg navbar-dark bg-${bg} ${fixed === 'top' ? 'fixed-top' : ''} ${fixed === 'bottom' ? 'fixed-bottom' : ''}`} data-testid={`${testId}`}>
      <div className="container-fluid">
        <a href="#/" className="navbar-brand">
          <OrdinoLogo></OrdinoLogo>
        </a>
        <a href="/app/" className="order-2 mx-3 mx-lg-0 ms-auto ms-lg-3 btn btn-light" data-testid="start-analysis-button">Start Analysis</a>
        ...
        <div className="order-1 navbar-collapse collapse" id="ordino-header-navbar-nav">
          {links &&
            <ul className="navbar-nav">
              {links.map(({text, page, faIcon}, i) => <li className="px-3 nav-item" key={i}><NavLink to={page} className="nav-link" data-testid={`${page}-link`} activeClassName="active">{faIcon && (<i className={`${faIcon} me-2`}></i>)}{text}</NavLink></li>)}
            </ul>
          }
        </div>
      </div>

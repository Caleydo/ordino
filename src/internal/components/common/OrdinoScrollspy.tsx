    ...
    <>
      ...
      <ul className="list-group d-none d-xxxl-block list-group-flush ordino-scrollspy-nav flex-column ms-4">
        {props.items.map((item) => {
          return (
            <a key={item.id} href={`#${item.id}`} onClick={scrollIntoView} className={`ps-0 mt-0 border-0 bg-transparent list-group-item list-group-item-action ${item.id === activeId ? 'active' : ''}`} data-testid={`${item.id}-scrollspy-link`}>{item.name}</a>
          );
        })}
      </ul>
    </>
